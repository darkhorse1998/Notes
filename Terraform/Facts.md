...
70. Vault provider in terraform helps to read, write and configure Hashicorp vault. Interacting with vault from terraform will persist any secrets that are read or written, in the state file.
71. Release cycle of provider plugins and terraform are independent. Version constraints can be used to determine which versions of the dependencies/plugins are used and compatible. After a version is selected for each dependency, terraform saves it in the lock file (.terraform.lock.hcl) and use the same version in future, even if there are new versions avaiable. This ensures that there are no compatibality issues between the code and the dependency versions.
72. To upgrade the version fo the dependencies, configure it in the provider block and then run `terraform init -upgrade`
73. At present (2024), the dependency lock file tracks only the provider dependencies/plugins. It doesn't track the versions of the remote modules. It will pick up the newest available module version.
